...
The elements of the total 4x4 transfer matrix :math:`\tensorbf{T}` can be used to determine the total reflectance and transittance for each s- and p- mode.
...
The transfer matrix method in PDGui is provided by modified version of the pyGTM :cite:`pygtm`.
...
This method has not been implemented for the scattering matrix formalism.
...
The 'Coherent' curve shows large amplitude oscillations before and after the restrahlung region of absorption.
...
With the transfer matrix method, it is common to see numerical problems in regions where the extinction coefficient is high and the dielectric is thick.  The numerical probles manifest themselves with either with sudden spikes in reflectance or with a 'Warning exponential overflow occured' message.
...
The transfer and scattering matrix methods agree well with other upto about 114 |cm-1|.  Above 180 |cm-1| the transfer matrix method is completely unstable, while the scattering matrix method works well.
