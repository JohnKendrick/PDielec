...
class CrystalOutputReader(GenericOutputReader):
    ...
    def _read_output_files(self):
        ...
        self.manage["lattice"] = (
            re.compile(" DIRECT LATTICE VECTORS CARTESIAN COMPONENTS"),
            self._read_lattice_vectors,
        )
        self.manage["fractional"] = (
            re.compile(" ATOMS IN THE ASYMMETRIC UNIT"),
            self._read_fractional_coordinates,
        )
        self.manage["bornCharges"] = (
            re.compile(" ATOMIC BORN CHARGE TENSOR"),
            self._read_born_charges,
        )
        self.manage["eigenvectors"] = (
            re.compile(" NORMAL MODES NORMALIZ"),
            self._read_eigenvectors,
        )
        self.manage["staticIonic"] = (
            re.compile(" SUM TENSOR OF THE VIBRATIONAL CONTRIBUTIONS TO "),
