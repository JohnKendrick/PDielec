...
# ---------------------------------------------------------------------------
# Stubs: FS16b validation (thick-layer GTM convergence)
# ---------------------------------------------------------------------------

class TestFS16ValidationStub:
    """Validation stub for FS16b: thickness convergence with real GTM.

    These tests are marked skip because they require running the GTM
    calculator at multiple thicknesses.  They are not run on every commit.
    Fill in when the Phase 3 OpticalChannelResolver is complete.

    Expected behaviour (from design doc FS16):
    - A polar mode computed via _coherent_amplitude with real GTM fields
      and increasing L should converge to the LO tensor in backscattering
      and the TO tensor in forward scattering, without any geometry switch.
    - The convergence rate should be ~1/L.
    """

    @pytest.mark.skip(reason="Validation test — requires GTM; not run on every commit")
    def test_gtm_backscatter_converges_to_LO_with_thickness(self):
        """Real GTM backscatter amplitude/L converges to LO tensor contribution."""
        raise NotImplementedError(
            "Set up a ZnO single-layer GTM stack at normal-incidence backscattering. "
            "Compute A(L)/L for L = 100 nm, 1 um, 10 um, 100 um. "
            "Verify convergence to the A1(LO) intensity and 1/L residual decay."
        )

    @pytest.mark.skip(reason="Validation test — requires GTM; not run on every commit")
    def test_gtm_forward_scatter_converges_to_TO_with_thickness(self):
        """Real GTM forward scatter amplitude/L converges to TO tensor contribution."""
        raise NotImplementedError(
            "Same setup as above but with collection_side='substrate'. "
