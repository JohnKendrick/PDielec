...
class PowderScenarioTab(ScenarioTab):
    ...
    def __init__(self, parent, debug=False):
        ...
        if "Material defined manually" in self.settings["Matrix"]:
            ...
            self.matrixMaterial = Materials.Constant(
                "manual",
                permittivity=self.settings["Matrix permittivity"],
                density=self.settings["Matrix density"],
            )
        else:
            self.materialDefinedManually = False
    ...
    def update_vf_sb(self):
        ...
        vf1 = (
            (1.0 - self.settings["Bubble volume fraction"])
            * (mf1 / mf2)
            * (rho2 / rho1)
            / (1 + (mf1 / mf2) * (rho2 / rho1))
        )
    ...
    def on_matrix_cb_activated(self, index):
        ...
        debugger.print(
            ...
            "on matrix combobox activated",
            self.matrix_cb.currentText(),
        )
    ...
    def on_permittivity_i_sb_changed(self, value):
        ...
        debugger.print(
            self.settings["Legend"],
            "on imaginary permittivity line edit changed",
            value,
        )
        self.refresh()
        self.refreshRequired = True
    ...
    def calculate(self, vs_cm1):
        ...
        hkl = [
            self.settings["Unique direction - h"],
        ...
        partial_function = partial(
            Calculator.solve_effective_medium_equations,
            method,
            volume_fraction,
            particle_size_mu,
            particle_sigma_mu,
