...
class OpenGLWidget(QOpenGLWidget):
    ...
    def __init__(self, parent, debug=False):
        """Initialise an OpenGLWidget object.
    ...
    def save_movie(self, filename):
        ...
        debugger.print("save_movie", filename)
        for _i in range(2 * self.number_of_phases):
            self.timeoutHandler()
    ...
    def startAnimation(self):
        """Start or restart an animation timer.
    ...
    def paintGL(self):
        ...
        self.background_colour = (
            np.array(self.viewerTab.settings["Background colour"]) / 255.0
        )
        glClearColor(*self.background_colour)
        glPushMatrix()
        glTranslatef(
            -self.rotation_centre[0], -self.rotation_centre[1], -self.rotation_centre[2],
        )
        self.drawSpheres()
        self.drawCylinders()
        if self.show_arrows:
    ...
    def drawArrows(self):
        ...
        for arrow, sphere in zip(self.arrows, self.spheres[self.current_phase], strict=False):
            pos = sphere.position
    ...
    def setProjectionMatrix(self):
        ...
        glLoadIdentity()
        orthox = (
            1.1 * self.image_size * self.my_width / min(self.my_width, self.my_height)
        )
        orthoy = (
            1.1 * self.image_size * self.my_height / min(self.my_width, self.my_height)
        )
    ...
    def defineLights(self):
        ...
        if self.light_switches is None:
            ...
            for position, _lightOn, light in zip(
                self.light_positions, self.light_switches, self.lights, strict=False,
            ):
                glLightfv(
                    light,
                    GL_AMBIENT,
                    [
                        self.ambientLightFactor,
                        self.ambientLightFactor,
                        self.ambientLightFactor,
                        1.0,
                    ],
        ...
        for lightOn, light in zip(self.light_switches, self.lights, strict=False):
            if lightOn:
                glEnable(light)
            else:
                glDisable(light)
