...
class MainTab(QWidget):
    ...
    def __init__(self, parent, program, filename, excelfile, debug=False):
        ...
        maximum_button_width = max(
            script_button_size.width(),
            excel_button_size.width(),
            file_button_size.width(),
        )
        maximum_button_height = max(
            script_button_size.height(),
            excel_button_size.height(),
            file_button_size.height(),
        )
        ...
        if os.path.exists(filename):
            ...
            if self.notebook.overwriting:
                ...
                answer = QMessageBox.question(
                    self,
                    "",
                    "Script file already exists.  Continue?",
    ...
    def read_output_file(self):
        ...
        if not os.path.isfile(filename):
            QMessageBox.about(
                ...
                "The filename for the output file to be processed is not correct: "
                + filename,
            )
        ...
        if self.reader is None:
            ...
            QMessageBox.about(
                self,
                "Processing output file",
                "A reader has not be created for this filename: " + filename,
            )
        ...
        if self.debug:
            ...
            try:
                ...
                QMessageBox.about(
                    self,
                    "Processing output file",
                    "Error on reading the output file using read_output(): " + filename,
                )
        ...
        if len(self.reader.unit_cells) == 0:
            ...
            QMessageBox.about(
                self,
                "Processing output file",
                "The output file has no unit cells in it: " + filename,
            )
    ...
    def on_file_button_clicked(self):
        ...
        filename, myfilter = QFileDialog.getOpenFileName(
            self,
            "Open MM/QM Output file",
            "",
            "Abinit (*.out);;Castep (*.castep);;Crystal 14 (*.out);;Experiment (*.exp);;Gulp (*.gout);;Phonopy (OUTCAR*);;QE (*.dynG);;VASP (OUTCAR*);;PDGui (*.py);;All files(*)",
            selfilter,
        )
