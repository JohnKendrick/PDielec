...
class MainTab(QWidget):
    ...
    program : str
        The name of the quantum mechanics or molecular mechanics program related to the data being analyzed or
        processed.
    filename : str
        The path to the output file generated by the specified `program`. This file contains the results to be analyzed.
    excelfile : str
        The name of an Excel file where results are stored. If specified, results will be written to this file
        upon program exit.
    debug : bool, optional
    ...
    on_excel_button_clicked()
        Handles actions to be taken when the 'Save results' button is clicked, typically involving writing data
        to an Excel file.
    on_calculation_button_clicked()
        Initiates the processing of the output file and the refreshing of related UI components and data
        representations.
    write_spreadsheet()
        Writes the current settings and analysis results to an Excel file specified by `excelfile`.
    read_output_file()
        Reads and processes the output file specified at initialization, updating UI and data structures based
        on its content.
    ...
    on_file_button_clicked()
        Opens a file dialog for the user to select a new output or script file, and updates settings based on
        the selection.
    on_file_le_return()
        Triggers actions similar to `on_file_button_clicked` but is specifically for handling manual text entry
        in the file path input field.
    ...
    def on_file_le_return(self):
        ...
        - Depending on the program associated with the filename, it might read the script or mark the interface
        for refresh and calculation.
        - The actual actions, especially involving UI updates (e.g., setting window title, deleting scenarios,
        reading script) or marking for refresh and calculation, are dependent on the context and design of the
        surrounding application and GUI structure.
        - Specific behaviors, such as how 'pdgui' program type is specially handled or the resetting of certain
