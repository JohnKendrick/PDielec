...
class AnalysisTab(QWidget):
    ...
    def on_vmin_changed(self):
        ...
        if vmin < vmax:
            ...
            debugger.print(
                "on_vmin_changed restricting value to",
    ...
    def on_vmax_changed(self):
        ...
        if vmax > vmin:
            ...
            debugger.print(
                "on_vmax_changed restricting value to",
    ...
    def calculate(self):
        ...
        mass_weighted_normal_modes = (
            self.notebook.settingsTab.mass_weighted_normal_modes
        )
        ...
        for imode, mode in enumerate(mass_weighted_normal_modes):
            for index, old_index in enumerate(self.original_atomic_order):
                ...
                self.new_normal_modes[imode, i + 0] = (
                    self.new_mass_weighted_normal_modes[imode, i + 0]
                    / math.sqrt(masses[index])
                )
                self.new_normal_modes[imode, i + 1] = (
                    self.new_mass_weighted_normal_modes[imode, i + 1]
                    / math.sqrt(masses[index])
                )
                self.new_normal_modes[imode, i + 2] = (
                    self.new_mass_weighted_normal_modes[imode, i + 2]
                    / math.sqrt(masses[index])
                )
        # Calculate the distribution in energy for the normal modes
        mode_energies = Calculator.calculate_energy_distribution(
            self.cell_of_molecules,
            self.frequencies_cm1,
            self.new_mass_weighted_normal_modes,
        )
