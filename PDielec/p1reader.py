...
def print_help():
    print('p1reader -program program [-eckart] [-neutral] [-nocalculation] [-hessian symm] [-masses average] [-version] filename', file=sys.stderr)
...
def main():
    ...
    prog = program
    if program == "auto":
        prog = find_program_from_name(filename)
    print('  Analysing {} generated by {}'.format(filename,prog),file=sys.stderr)
    calling_parameters = (filename, eckart, neutral, mass_definition, mass_dictionary, global_no_calculation, prog, hessian_symmetrisation, qmprogram, debug) 
    results = read_a_file(calling_parameters)
