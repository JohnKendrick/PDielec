...
def main():
    ...
    if len(sys.argv) <= 1:
        ...
        print(
            "         Compare the two excel files for any significant changes",
            file=sys.stderr,
        )
        print("         Numbers f1 and f2 in each file are compared", file=sys.stderr)
        print(
            "         an error is flagged if 2*abs(f1-f2)/(f1+f2+2) > threshold",
            file=sys.stderr,
        )
    ...
    for sheet in sheets:
        ...
        if max_rows1 != max_rows2:
            print(
                "Error - the number rows in the sheet are not the same",
                sheet,
                max_rows1,
                max_rows2,
            )
        ...
        if max_columns1 != max_columns2:
            print(
                "Error - the number columns in the sheet are not the same",
                sheet,
                max_columns1,
                max_columns2,
            )
