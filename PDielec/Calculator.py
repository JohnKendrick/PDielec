...
def fibonacci_sphere(samples=1, randomize=True):
    ...
    points = []
    offset = 2.0 / samples
    increment = np.pi * (3.0 - math.sqrt(5.0))
...
def ionic_permittivity(mode_list, oscillator_strengths, frequencies, volume):
    ...
    for imode in mode_list:
        permittivity = permittivity + oscillator_strengths[imode] / (
            frequencies[imode] * frequencies[imode]
        )
    # end for
...
def infrared_intensities(oscillator_strengths):
    ...
    for imode, strength in enumerate(oscillator_strengths):
        ...
        intensities[imode] = (
            intensities[imode] + strength[0, 0] + strength[1, 1] + strength[2, 2]
        )
    # end for
...
def absorption_from_mode_intensities(f, modes, frequencies, sigmas, intensities):
    ...
    for mode in modes:
        ...
        absorption = absorption + 2.0 * 4225.6 * icastep / np.pi * (
            sigma / (4.0 * (f - v) * (f - v) + sigma * sigma)
        )
    return absorption
...
def calculate_size_factor(x):
    ...
    if x < 1.0e-12:
        ...
        g2 = (
            1 + ix - (7.0 / 15.0) * x * x - complex(0, 2.0 * x * x * x / 15.0)
        ) * np.exp(-ix) - 1.0
        result = 1 - g1 - g2
    return result
...
def spherical_averaged_mie_scattering(
    ...
    for point in points_on_sphere:
        ...
        refractive_index = (
            calculate_refractive_index_scalar(rotated_dielec) / refractive_index_medium
        )
...
def mie_scattering(
    ...
    refractive_index = (
        calculate_refractive_index_scalar(einclusion) / refractive_index_medium
    )
    if size_distribution_sigma:
        # Calculate the integral of the forward scattering factors over the distribution
        s1_factors = []
...
def anisotropic_mie_scattering(
    ...
    for index in [0, 1, 2]:
        refractive_index = (
            calculate_refractive_index_scalar(rotated_dielec[index, index])
...
def bruggeman_minimise(eps1, eps2, shape, L, f2, size, epsbr):
    ...
    options = {"xtol": 1.0e-4, "ftol": 1.0e-4}
    sol = sc.minimize(
        _brug_minimise_tensor,
        variables,
        method="Powell",
        args=(eps1, eps2, shape, L, f1, size),
        options=options,
    )
    if not sol.success:
...
def solve_effective_medium_equations(
    ...
    if bubble_vf > 0.0:
        if np.abs(refractive_index.imag) > 1.0e-12:
            print(
                "Warning: only the real part of the support matrix permittivity will be used for Mie Scattering",
                file=sys.stderr,
            )
    ...
    if method == "balan":
        ...
        effdielec = anisotropic_mie_scattering(
            dielectric_medium.real,
            crystal_permittivity,
            shape,
            L,
            vf,
    ...
    # units are cm-1
    absorption_coefficient = (
        v_cm1 * 4 * np.pi * np.imag(refractive_index) * math.log10(math.e)
    )
...
def calculate_energy_distribution(cell, frequencies, normal_modes, debug=False):
    ...
    for _imode, mode in enumerate(normal_modes):
        ...
        for _imol, (ps, mask) in enumerate(
            ...
            mode_cm = (
                mode
                - np.dot(mode, ps[0]) * ps[0]
                - np.dot(mode, ps[1]) * ps[1]
                - np.dot(mode, ps[2]) * ps[2]
            )
            centre_of_mass_energy += total_energy - np.dot(mode_cm, mode_cm)
            # Project out molecular rotation of each molecule
            mode_cm = (
                mode
                - np.dot(mode, ps[3]) * ps[3]
                - np.dot(mode, ps[4]) * ps[4]
                - np.dot(mode, ps[5]) * ps[5]
            )
...
def reflectance_atr(ns, n0, theta, atrSPolFraction):
    ...
    rs = (
        -1.0
        * (n0 * costheta - cmath.sqrt(ns * ns - n0 * n0 * sintheta * sintheta))
        / (n0 * costheta + cmath.sqrt(ns * ns - n0 * n0 * sintheta * sintheta))
    )
    rp = (
        ns * ns / n0 * costheta - cmath.sqrt(ns * ns - n0 * n0 * sintheta * sintheta)
    ) / (ns * ns / n0 * costheta + cmath.sqrt(ns * ns - n0 * n0 * sintheta * sintheta))
