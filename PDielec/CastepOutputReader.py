...
class CastepOutputReader(GenericOutputReader):
    ...
    def _read_output_files(self):
        ...
        self.manage["cellcontents"] = (
            re.compile(" *Unit Cell"),
            self._read_cellcontents,
        )
        self.manage["pspots"] = (
            re.compile(" *Files used for pseudopotentials:"),
            self._read_pspot,
        )
        self.manage["masses"] = (
            re.compile(" *Mass of species in AMU"),
            self._read_masses,
        )
        self.manage["kpoints"] = (
            re.compile(" *Number of kpoints used"),
            self._read_kpoints,
        )
        self.manage["kpoint_grid"] = (
            re.compile(" *MP grid size for SCF"),
        ...
        self.manage["energies3"] = (
            re.compile("Dispersion corrected final energy"),
            self._read_energies3,
        )
        self.manage["energy_cutoff"] = (
            re.compile(" *plane wave basis set cut"),
            self._read_energy_cutoff,
        )
        self.manage["convergence"] = (
            re.compile(".*finished iteration"),
            self._read_convergence,
        )
        ...
        self.manage["opticalDielectric"] = (
            re.compile(" *Optical Permittivity"),
            self._read_dielectric,
        )
        ...
        self.manage["nbranches"] = (
            re.compile(" Number of branches"),
            self._read_nbranches,
        )
