...
def read_a_file(calling_parameters):
    ...
    if not no_calculation:
        ...
        if len(ignore_modes) == 0:
            for mode, intensity in enumerate(intensities):
                ...
                # ignore modes with low real frequency
                    ...
                    np.real(modified_frequencies_cm1[mode]) / wavenumber
                    < threshold_frequency
                ) or abs(np.imag(modified_frequencies_cm1[mode])) / wavenumber > 1.0e-6:
                    ignore_modes.append(mode)
                # end if intensity
            # end for
        # end if len()
        ...
        crystalPermittivity = DielectricFunction.DFT(
            mode_list,
            modified_frequencies * wavenumber,
            sigmas,
    ...
    string = (
        string + "," + c11 + "," + c22 + "," + c33 + "," + c44 + "," + c55 + "," + c66
    )
...
def main():
    ...
    if global_no_calculation:
        ...
        print(
            "  The frequencies (if present) will come from the QM/MM program    ",
            file=sys.stderr,
        )
        if eckart:
        ...
        if mass_definition != "program":
            print(
                "  The average and isotope mass definitions will be ignored",
                file=sys.stderr,
            )
            mass_definition = "program"
        if mass_dictionary:
            print(
                "  The masses specified by the -mass directive will be ignored",
                file=sys.stderr,
            )
            mass_dictionary = {}
    else:
    ...
    if observables:
        print("Observables is no longer supported")
