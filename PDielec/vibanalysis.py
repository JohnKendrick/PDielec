    ...
    for i in range(len(data)):
        if "ORIENTATION OF MOLECULE IN FORCE CALCULATION" in data[i]:
            for j in range(i + 4, i + 4 + natoms):
                ...
                o.addAtom(
                    l[1].lower(),
                    np.array(list(map(float, l[2:5]))),
                    Masses[Symbols.index(l[1].lower())],
                )
            break
    ...
    for i in range(len(data)):
        if "$atoms" in data[i]:
            ...
            for j in range(i + 2, i + 2 + natoms):
                ...
                o.addAtom(
                    l[0].lower(),
                    np.array(list(map(float, l[2:5]))) * Bohr2Ang,
                    float(l[1]),
                )
            break
    # read frequency list
...
def modeStr(mode, symbs):
    ...
    if mode[0] == 1:  # bond
        o = "BOND %s%d %s%d" % (
            symbs[mode[1]].capitalize(),
            mode[1] + 1,
            symbs[mode[2]].capitalize(),
            mode[2] + 1,
        )
    elif mode[0] == 2:  # ANGLE
        o = "ANGLE %s%d %s%d %s%d" % (
            ...
            symbs[mode[2]].capitalize(),
            mode[2] + 1,
            symbs[mode[3]].capitalize(),
            mode[3] + 1,
        )
    elif mode[0] == 3:  # OUT OF PLANE
        o = "OUT %s%d %s%d %s%d %s%d" % (
            symbs[mode[1]].capitalize(),
            mode[1] + 1,
            symbs[mode[2]].capitalize(),
            mode[2] + 1,
            symbs[mode[3]].capitalize(),
            mode[3] + 1,
            symbs[mode[4]].capitalize(),
            mode[4] + 1,
        )
    elif mode[0] == 4:  # TORSION
        o = "TORSION %s%d %s%d %s%d %s%d" % (
            symbs[mode[1]].capitalize(),
            mode[1] + 1,
            symbs[mode[2]].capitalize(),
            mode[2] + 1,
...
def makeIC(o, useric=[]):
    ...
    for i in range(len(lbonds)):
        ...
        for j in range(i + 1, len(lbonds)):
            ...
            if len(set1 & set2) == 1:
                ...
                centred[common] += 1
                if Opts["doAutoSel"]:
                    if (candidate not in langles) and (
        ...
        for j in range(i + 1, len(langles)):
            ...
            if len(set1 & set2) == 2:
                ...
                # now the torsions...
                    ...
                    candidate = []
                    if (langles[i][1] == langles[j][0]) and (
                        langles[j][1] == langles[i][0]
                    ):
                        candidate = [
                            langles[i][2],
                            langles[i][1],
                    ...
                    elif (langles[i][1] == langles[j][0]) and (
                        langles[j][1] == langles[i][2]
                    ):
                        candidate = [
                            langles[i][0],
                            langles[i][1],
                    ...
                    elif (langles[i][1] == langles[j][2]) and (
                        langles[j][1] == langles[i][0]
                    ):
                        candidate = [
                            langles[i][2],
    ...
    for i in range(len(ltors)):
        ...
        for j in [
            ltors[i][ii] for ii in [0, -1]
        ]:  # these two define the angle between planes
            for k in range(3):
                gp = np.copy(geo)
                gp[j, k] += delta
