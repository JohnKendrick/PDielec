...
The *SingleCrystal tab* enables the calculation of the optical behaviour of a single crystal (thick slab or film).  The method uses the pyGTM code available on GitHub :cite:`pygtm`.  This code implements a generalised transfer matrix method described by Passler et al. :cite:`Passler2020` and that builds on previous publications :cite:`Passler2017,Passler2017a`.
...
It is common to see numerical problems in regions where the extinction coefficient is high and the dielectric is thick.  The numerical probles manifest themselves with sudden spikes in reflectance and indicate that thinner samples should be considered.
To address these numerical problems an option has been included in PDGui which allows thick films to be subdivided (sliced) into many thin films.
If the user experiences numerical issues, changes in film thickness and the slicing technique may help to alleviate the problem.
