...
class FitterTab(QWidget):
    def __init__(self, parent, debug=False):   
        ...
        self.settings['Frequency shift'] = False
        self.settings['Frequency shift factor'] = 1.0
        self.settings['Optimise volume fraction'] = False
        self.settings['Volume fraction'] = self.notebook.scenarios[0]:settings['Volume fraction']
        self.useSpectrumError = True
...
#        form.addRow(label, hbox)
    ...
    def optimiseFit(self):
        ...
        # Append a shifting option
        if self.settings['Frequency shift']:
            initial_point.append(self.settings['Frequency shift factor'])
        # Append a optimise volume fraction option
        if self.settings['Optimise volume fraction']:
            initial_point.append(self.settings['Volume fraction'])
            self.notebook.scenarios[0]:settings['Mass or volume fraction'] = 'volume'
        nvariables = len(initial_point)
        if nvariables > 0:
            if self.useSpectrumError:
                final_point = minimize(self.optimiseSpectrumErrorFunction, initial_point, method='nelder-mead', options={'xtol':1.0, 'disp':False, 'maxiter':self.settings['Number of iterations']} )
            else:
                final_point = minimize(self.optimiseCrossCorrelationFunction, initial_point, method='nelder-mead', options={'xtol':1.0, 'disp':False, 'maxiter':self.settings['Number of iterations']} )
        else: 
             print('No sigmas have been selected for optimisation')
             final_point = []
        return final_point

    def optimiseSpectrumErrorFunction(self,variables) :
        # Determine the function to be optimised (minimised)
        # print('Current point in optimisation')
        # print(variables)
        debugger.print('optimiseSpectrumErrorFunction',variables)
        last_variable = 0
        if self.settings['Frequency scaling']:
            last_variable -= 1
            scaling_factor = variables[last_variable]
            self.settings['Frequency scaling factor'] = scaling_factor
        if self.settings['Frequency shift']:
            last_variable -= 1
            shift_factor = variables[last_variable]
            self.settings['Frequency shift factor'] = scaling_factor
        if self.settings['Optimise volume fraction']:
            last_variable -= 1
            self.settings['Volume fraction'] = variables[last_variable]
        sigmas = variables[:last_variable]
        ...
        debugger.print('optimiseCrossCorrelationFunction - xcorr0',self.xcorr0)
        return -1.0*self.xcorr0

    def optimiseCrossCorrelationFunction(self,variables) :
        # Determine the function to be optimised (minimised)
        # print('Current point in optimisation')
        # print(variables)
        debugger.print('optimiseCrossCorrelationFunction',variables)
        if self.settings['Frequency scaling']:
            sigmas = variables[:-1]
            self.settings['Frequency scaling factor'] = variables[-1]
        else:
            sigmas = variables
        for index,sigma in zip(self.fit_list,sigmas):
            self.sigmas_cm1[index] = sigma
    ...
    def calculateSpectrumError(self,scaling_factor):
        debugger.print('calculateSpectrumError',scaling_factor)
        # col1 contains the experimental spectrum
        col1 = np.array(self.experimental_absorption)
        # Normalise so the largest element is 1
        col1 = col1 / np.max(col1)
        # Set any elements below a threshold to 0
        col1[ col1 < threshold ] = 0.0
        # col2 contains the calculated spectrum, normalise with the same normal as col1
        col2 = np.array(self.calculated_absorptions[0]) / np.max(col1)
        # Set any elements below a threshold to 0
        col2[ col2 < threshold ] = 0.0
        diff = col1 - col2
