...
class ScenarioTab(QWidget):
    def __init__(self, parent, mainTab, settingsTab):   
    ...
    def change_greyed_out(self):
        ...
        if self.settings["shape"] == "Ellipsoid":
            ...
            print("Shape no recognised", self.settings["shape"])
    ...
    def refresh(self):
        # Update the legend
        if self.scenarioIndex is not None:
            self.legend_le.setText("Scenario {} legend".format(self.scenarioIndex))
        else:
            self.legend_le.setText("Scenario legend")
        # First see if we can get the reader from the mainTab
        self.reader = self.mainTab.reader
