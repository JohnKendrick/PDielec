...
=== Linux
Both Python 2.7 and Python 3 or supported.  Install Python using the package manager for your version of Linux.  In addition install python versions of yaml, numpy, scipy and matplotlib.  In Ubuntu they are installed by;

 sudo apt-get install yaml python-dev python-numpy python-scipy python-matplotlib
...
preader has several options including;
 ...
 preader -program qe `find . -name \*.dynG` > summary.csv
